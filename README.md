...
| Probe router (Arm 3) | _pending — wired in last_ |
...
python scripts/demo_ui.py --backend rag    # Arm 2 (needs Modal embeddings precomputed)
...
modal deploy modal_app/precompute_embeddings.py      # Arm 2 embedder
modal run    modal_app/precompute_embeddings.py      # one-time GPU embeddings -> Volume
...
| 2/4 Probes + ProbeRoutedFlow (Arm 3) | pending — flip `ROUTER_BACKEND=probe` after merge |
